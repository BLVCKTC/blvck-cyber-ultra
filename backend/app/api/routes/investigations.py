...
@router.post('', response_model=InvestigationRead, status_code=201, dependencies=[Depends(require_permission('forensics.manage'))])
def create(payload: InvestigationCreate, membership: Membership = Depends(get_active_membership), service: InvestigationService = Depends(svc)):
    return service.create(membership.tenant_id, payload)
 
@router.get('/{investigation_id}', response_model=InvestigationRead, dependencies=[Depends(require_permission('forensics.view'))])
...
@router.patch('/{investigation_id}', response_model=InvestigationRead, dependencies=[Depends(require_permission('forensics.manage'))])
...
@router.get('/{investigation_id}/evidence', response_model=list[EvidenceRead], dependencies=[Depends(require_permission('forensics.view'))])
...
@router.post('/{investigation_id}/evidence', response_model=EvidenceRead, status_code=201, dependencies=[Depends(require_permission('forensics.manage'))])
