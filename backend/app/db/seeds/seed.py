...
class SeedRunner:
    ...
    def seed_permissions(self) -> None:
        ...
        print(f"Updated permissions: {updated}")
    ...
    def seed_role_permissions(self) -> None:
        ...
        print(f"Created role permissions: {created}")
        print(f"Existing role permissions: {existing_count}")
    ...
    def seed_tenant_roles(self) -> None:
        ...
        if not tenants:
            print("No tenants found. Skipping tenant roles.")
            return
        ...
        print(f"Created tenant roles: {created}")
        print(f"Updated tenant roles: {updated}")
    ...
    def seed_tenant_role_permissions(self) -> None:
        print("Seeding tenant role permissions...")

        global_permissions = self.db.query(RolePermission).all()

        role_permission_map: dict[str, list[int]] = {}

        for item in global_permissions:
            role_permission_map.setdefault(item.role_key, []).append(
                item.permission_id
            )
        ...
        if not tenants:
            print("No tenants found. Skipping tenant role permissions.")
            return
        ...
        for tenant in tenants:
            ...
            tenant_roles = (
                self.db.query(TenantRole)
                .filter(TenantRole.tenant_id == tenant.id)
                .all()
            )

            for tenant_role in tenant_roles:
                permission_ids = role_permission_map.get(
                    tenant_role.key,
                    []
                )
                ...
                for permission_id in permission_ids:
                    existing = (
                        ...
                        .filter(
                            TenantRolePermission.tenant_role_id == tenant_role.id,
        ...
        print(f"Created tenant role permissions: {created}")
        print(f"Existing tenant role permissions: {existing_count}")
