...
class DetectionRule(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "version > 0",
            name="ck_detection_rules_version_positive",
        ),
        CheckConstraint(
            "rule_type IN "
            "('threshold','query','correlation','behavioral','sigma')",
        ...
        CheckConstraint(
            "status IN "
            "('draft','testing','backtested','canary','approved',"
            "'production','monitored','tuned','retired')",
            name="ck_detection_rules_status",
        ),
        Index(
            "ix_detection_rules_tenant_status",
            "tenant_id",
            "status",
        ),
        Index(
            "ix_detection_rules_tenant_enabled",
            "tenant_id",
            "enabled",
        ),
        Index(
            "ix_detection_rules_tenant_type",
            "tenant_id",
            "rule_type",
        ),
        Index(
            "ix_detection_rules_tenant_severity",
            "tenant_id",
            "severity",
        ),
    )
    ...
    mitre_technique_ids: Mapped[list[str]] = mapped_column(
        ARRAY(String(50)),
        nullable=False,
        default=list,
        server_default=text("'{}'"),
    ...
    mitre_tactic_ids: Mapped[list[str]] = mapped_column(
        ARRAY(String(50)),
        nullable=False,
        default=list,
        server_default=text("'{}'"),
