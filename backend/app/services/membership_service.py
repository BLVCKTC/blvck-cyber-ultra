...
class MembershipService:
    ...
    def _get_required_user(self, user_id: int):
        user = self.users.get(user_id)
        if user is None:
            raise HTTPException(status_code=404, detail="user_not_found")
        return user

    def _get_required_membership(self, membership_id: int) -> Membership:
    ...
    def _assert_not_last_owner(
        ...
        *,
        ...
        - If removing an OWNER (next_role != OWNER), raise when count == 1
        - If updating OWNER -> OWNER, safe
        - If current_role != OWNER, safe
        """
        if current_role != MembershipRole.OWNER:
            return

        # If explicitly staying OWNER, allowed.
        if next_role == MembershipRole.OWNER:
            return

        owner_count = self.memberships.count_by_role(tenant_id, MembershipRole.OWNER)
        if owner_count == 1:
            # Distinguish error message when possible
    ...
    def add_member(self, tenant_id: str, payload: MembershipCreate) -> Membership:
        ...
        try:
            ...
            # Defensive: repo also checks existence
            raise HTTPException(status_code=409, detail="user_already_member")

    def update_member(self, membership_id: int, payload: MembershipUpdate) -> Membership:
