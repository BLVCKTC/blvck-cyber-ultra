...
const statuses: Array<AlertStatus | 'all'> = [
  'all',
  'new',
  'open',
  'acknowledged',
...
export function AlertsTable({ tenantId }: { tenantId: string }) {
  ...
  const params = useMemo(
    ...
    [query, severity, status, offset],
  )
  const { data, error, isLoading, mutate } = useSWR(
    ['alerts', tenantId, params],
    () => getAlerts(tenantId, params),
  )
    ...
    <section className="rounded-lg border border-border bg-card">
      <div className="flex flex-wrap items-center gap-3 border-b border-border p-3">
        ...
        <select
          ...
          onChange={(event) =>
            updateFilter(
              setSeverity as (value: never) => void,
              event.target.value,
            )
          }
          className="h-9 rounded-md border border-border bg-background px-3 text-sm capitalize text-foreground"
        >
      ...
      {error && (
        <div
          className="flex items-center justify-between gap-3 border-b border-border p-4 text-sm text-destructive"
          role="alert"
        >
          <span>Unable to load alerts: {error.message}</span>
          <button
            type="button"
            onClick={() => mutate()}
              ...
              data?.items.map((alert: Alert) => (
                ...
                >
                  <td className="px-4 py-3">
                    <Link
                      href={`/dashboard/${tenantId}/alerts/${alert.id}`}
                      className="block"
                    >
      ...
      <div className="flex items-center justify-between border-t border-border px-4 py-3 text-xs text-muted-foreground">
        <span>{data ? `${data.total} total alerts` : 'Loading alerts'}</span>
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            disabled={!data || offset + limit >= data.total || isLoading}
...
export function AlertKpis({
  ...
  const open =
    data?.items.filter(
      (alert) =>
        !['resolved', 'suppressed', 'false_positive'].includes(alert.status),
    ).length ?? 0
  const critical =
    data?.items.filter((alert) => alert.severity === 'critical').length ?? 0
  return (
    <div className="flex gap-6 text-xs text-muted-foreground">
      <span>
        <strong className="text-foreground">{data?.total ?? '—'}</strong> total
      </span>
      <span>
        <strong className="text-foreground">{open}</strong> open
      </span>
      <span>
        <strong className="text-foreground">{critical}</strong> critical
      </span>
    </div>
  )
}
