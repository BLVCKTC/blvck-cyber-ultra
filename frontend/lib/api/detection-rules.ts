...
async function request<T>(tenantId: string, path = '', init?: RequestInit): Promise<T> { const response = await authenticatedFetch(`${tenantApiPath(tenantId, 'detection-rules')}${path}`, init); if (!response.ok) throw new Error(await response.text() || `Request failed (${response.status})`); return response.status === 204 ? undefined as T : response.json() }
export function getDetectionRules(tenantId: string, params: Record<string, string | number | boolean | undefined> = {}) { const query = new URLSearchParams(); Object.entries(params).forEach(([k,v]) => v !== undefined && query.set(k, String(v))); return request<DetectionRuleList>(tenantId, query.size ? `?${query}` : '') }
export const getDetectionRule = (tenantId: string, id: string) => request<DetectionRule>(tenantId, `/${encodeURIComponent(id)}`)
 export const createDetectionRule = (tenantId: string, input: DetectionRuleInput) => request<DetectionRule>(tenantId, '', { method: 'POST', body: JSON.stringify(input), headers: { 'Content-Type': 'application/json' } })
 export const updateDetectionRule = (tenantId: string, id: string, input: Partial<DetectionRuleInput>) => request<DetectionRule>(tenantId, `/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify(input), headers: { 'Content-Type': 'application/json' } })
 export const deleteDetectionRule = (tenantId: string, id: string) => request<void>(tenantId, `/${encodeURIComponent(id)}`, { method: 'DELETE' })
